...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  // change anytime vars
  { &g_timelimit, "timelimit", "45", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//CVAR_SERVERINFO
  { &g_suddenDeathTime, "g_suddenDeathTime", "30", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//CVAR_SERVERINFO
  { &g_suddenDeathMode, "g_suddenDeathMode", "0", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },//CVAR_SERVERINFO
  { &g_suddenDeath, "g_suddenDeath", "0", CVAR_NORESTART, 0, qtrue },//CVAR_SERVERINFO

  { &g_enterString, "g_enterString", "", 0, 0, qfalse },	
  { &g_synchronousClients, "g_synchronousClients", "0", CVAR_SYSTEMINFO, 0, qfalse  },

  { &g_friendlyFire, "g_friendlyFire", "0", CVAR_ARCHIVE, 0, qtrue  },//CVAR_SERVERINFO
  ...
  { &g_friendlyBuildableFire, "g_friendlyBuildableFire", "0", CVAR_ARCHIVE, 0, qtrue  },//CVAR_SERVERINFO
  ...
  { &g_humanBuildPoints, "g_humanBuildPoints", DEFAULT_HUMAN_BUILDPOINTS, 0, 0, qfalse  },//CVAR_SERVERINFO
  { &g_alienBuildPoints, "g_alienBuildPoints", DEFAULT_ALIEN_BUILDPOINTS, 0, 0, qfalse  },//CVAR_SERVERINFO
  ...
  { &g_rageFilter, "g_rageFilter", "0", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_allowShare, "g_allowShare", "0", CVAR_ARCHIVE, 0, qfalse},//CVAR_SERVERINFO
...
int QDECL SortRanks( const void *a, const void *b )
{
  ...
  // then sort by score
	if( ca->ps.persistant[ PERS_SCORE ] > cb->ps.persistant[ 
PERS_SCORE ] )
		return -1;
	else if( ca->ps.persistant[ PERS_SCORE ] < cb->ps.persistant[ 
PERS_SCORE ] )
    return 1;
  else
    return 0;
...
void G_CountSpawns( void )
{
  ...
  {
    if(level.numAlienSpawns == 0 && level.alienTeamLocked == qfalse)
    {
      trap_SendServerCommand(-1,"print \"^1Aliens have no spawns. Team locked.\n\"");
      level.alienTeamLocked = qtrue;
    }
    if(level.numHumanSpawns == 0 && level.humanTeamLocked == qfalse)
    {
      trap_SendServerCommand(-1,"print \"^5Humans have no spawns. Team locked.\n\"");
      level.humanTeamLocked = qtrue;
    }
  }
  ...
  if( level.voteExecuteTime && level.voteExecuteTime < level.time )
  {
    ...
    if( !Q_stricmp( level.voteString, "suddendeath" ) )
    {
      level.suddenDeathBeginTime = level.time + ( 1000 * g_suddenDeathVoteDelay.integer ) - level.startTime;
		...
		g_suddenDeathTime.integer = level.suddenDeathBeginTime;
		
      level.voteString[0] = '\0';

      if( g_suddenDeathVoteDelay.integer )
        trap_SendServerCommand( -1, va("cp \"^1Sudden Death^7 will begin in %d seconds\n\"", g_suddenDeathVoteDelay.integer  ) );
    }
...
void CheckCountdown( void )
{
  ...
  lastmsg = level.time;
  if( timeleft > 0 )
    trap_SendServerCommand( -1, va( "cp \"^5Warmup Time:^7\n^%i----- ^7%i ^%i-----\"", timeleft % 7, timeleft, timeleft % 7 ) );
  else if( timeleft == 0 ) 
