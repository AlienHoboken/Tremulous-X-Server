...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  if( pm->ps->pm_type == PM_GRABBED )
    modifier *= LEVEL1_GRAB_SPEED_MOD;//0.0f;
...
static qboolean PM_CheckWallJump( void )
{
  ...
  /*if(pm->ps->pm_type == PM_GRABBED)//Even walljumper humans shouldnt 
jump when grabbed
  {
    pm->cmd.upmove = 0;
    return qfalse;
...
void PM_UpdateViewAngles( playerState_t *ps, const usercmd_t *cmd )
{
  ...
  if( ps->pm_type == PM_GRABBED && !BG_InventoryContainsUpgrade( 
UP_BATTLESUIT, ps->stats ) )
  {
    vec3_t  dir, angles;
