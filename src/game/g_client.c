  ...
  if( g_clientUpgradeNotice.integer )
  {
    if( !Q_stricmp( ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" ) )
    {
      trap_SendServerCommand( client->ps.clientNum, va( "print \"^1Your client is out of date. Updating your client will allow you to "
        ...
        "at ^2http://xserverx.com/backport/^1 and reconnect. \n\"" ) );
    }
  }

		trap_SendServerCommand( client->ps.clientNum, va("cp \"%s^7, %s\"", g_enterString.string, client->pers.netname ));
...
void ClientSpawn( gentity_t *ent, gentity_t *spawn, vec3_t origin, vec3_t angles )
{
  ...
  if( client->sess.sessionTeam != TEAM_SPECTATOR )
  {
    /*if(ent->client->pers.teamSelection == PTE_ALIENS) {
      int hp;
      float ratio;
      if(level.numAlienClients == 0) {
        hp = BG_FindHealthForClass( ent->client->pers.classSelection );
      } else {
        ratio = ((float)level.numHumanClients / (float)level.numAlienClients);
        if((ratio < 1.2 && ratio > 0.8) || ratio == 0.0f)//ignore small difference in ratio
        {
          ratio = 1.0f;
        }
        hp = (float)BG_FindHealthForClass( ent->client->pers.classSelection ) * ratio;
        if(hp < 1)
          hp = 1;
      }
