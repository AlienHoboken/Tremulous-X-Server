...
void buildFire( gentity_t *ent, dynMenu_t menu )
{
  if( ( ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) > BA_NONE )
  {
    ...
    if( G_BuildIfValid( ent, ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) )
    {
      ...
      //ent->client->ps.stats[ STAT_BUILDABLE ] = BA_NONE; //Allow build spam.
...
void CheckGrabAttack( gentity_t *ent )
{
  ...
  G_UnlaggedOn(ent, ent->client->ps.origin, LEVEL1_GRAB_RANGE);
  trap_Trace( &tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT );
  G_UnlaggedOff();
  if( tr.surfaceFlags & SURF_NOIMPACT )
    return;
...
void marauderspit(gentity_t *ent)
{
  ...
  if(traceEnt->takedamage && traceEnt->client)
  {
    tent = G_TempEntity(tr.endpos,EV_MISSILE_HIT);
    tent->s.eventParm = DirToByte(tr.plane.normal);
    tent->s.weapon = WP_ABUILD2;
    tent->s.generic1 = WPM_TERTIARY;
  }
  else
  {
    tent = G_TempEntity(tr.endpos,EV_MISSILE_MISS);
  ...
  if(traceEnt->takedamage)
    G_Damage(traceEnt,ent,ent,forward,tr.endpos,LEVEL2_SPIT_DMG,0,MOD_SLOWBLOB);
}
