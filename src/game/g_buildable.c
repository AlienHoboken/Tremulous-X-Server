...
void HMedistat_Think( gentity_t *self )
{
  ...
  if( self->spawned )
  {
    ...
    else if( self->enemy && self->enemy->health > 0 ) //heal!
    {
      ...
      {
        self->enemy->health = self->enemy->client->ps.stats[ STAT_MAX_HEALTH ];//Limit
...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin )
{
  ...
  //Don't allow unreplacable buildings to be built during sd
  ...
  // check nobuild zones
  if( nobuild_check( entity_origin ) >= 0 && ent->client->pers.teamSelection == PTE_HUMANS )
...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  ...
  switch( buildable )
  {
    ...
    case BA_A_HOVEL:
      ...
      //built->use = AHovel_Use; Hovel too glitchy
  ...
  if( builder && builder->client ) // log ingame building only
  {
	...
	if( builder->client ) {
		...
		{
			G_TeamCommand( PTE_HUMANS, va("cp \"^5Reactor^7 built by %s\n^7Buildlog #%i\n\" 1",builder->client->pers.netname,new->ID));
			G_TeamCommand( PTE_HUMANS, va("print \"^7Reactor built by %s^7. Buildlog #%i\n\" 1",builder->client->pers.netname,new->ID));

		}
		else if(buildable == BA_A_OVERMIND)
		{
			G_TeamCommand( PTE_ALIENS, va("cp \"^1Overmind^7 built by %s\nBuildlog #%i\n\" 1",builder->client->pers.netname,new->ID));
			G_TeamCommand( PTE_ALIENS, va("print \"^7Overmind built by %s^7.Buildlog #%i\n\" 1",builder->client->pers.netname,new->ID));
...
void G_BaseSelfDestruct( pTeam_t team )
{
	...
	for( i = MAX_CLIENTS; i < level.num_entities; i++ )
	{
		...
		if( team == PTE_ALIENS && ent->biteam != BIT_ALIENS )
			continue;
		G_Damage( ent, NULL, NULL, NULL, NULL, 20000, 0, MOD_SUICIDE );
	}
	...
	for( j = 0; j < level.maxclients; j++ )
	{
		p = &level.clients[ j ];
		vic = &g_entities[ j ];    
		if( p->pers.teamSelection == team)
		{
			if (vic->client->ps.stats[ STAT_HEALTH ] > 0) {
			vic->client->ps.stats[ STAT_HEALTH ] = 
vic->health = 0;
			AP( va( "print \"%s^7 has commited suicide\n\"", 
