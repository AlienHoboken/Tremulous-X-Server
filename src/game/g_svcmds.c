...
qboolean  ConsoleCommand( void )
{
  ...
  if( Q_stricmp( cmd, "alienWin" ) == 0 )
  {
    ...
    {
      if( e->s.modelindex == BA_H_SPAWN || e->s.modelindex == 
BA_H_REACTOR )
        G_Damage( e, NULL, NULL, NULL, NULL, 30000, 0, MOD_SUICIDE );
    }
  ...
  if( Q_stricmp( cmd, "humanWin" ) == 0 )
  {
    ...
    for( i = 1, e = g_entities + i; i < level.num_entities; i++, e++ )
    {
      if( e->s.modelindex == BA_A_SPAWN || e->s.modelindex == 
BA_A_OVERMIND )
        G_Damage( e, NULL, NULL, NULL, NULL, 30000, 0, MOD_SUICIDE );
    }
  ...
  if( g_dedicated.integer )
  {
	...
	else if( !Q_stricmp( cmd, "d" ) || !Q_stricmp( cmd, "say_devs" ))
    {
		G_Say( NULL, NULL, SAY_DEVS, ConcatArgs( 1 )  );
		return qtrue;
    }
